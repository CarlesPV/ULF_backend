...
/*
* Función para validar el dominio de los usuarios al registrarse.
*/
export const validateUserDomainOnCreate = functions.auth.user().onCreate(async (user) => {
    const email = user.email;
    if (!email) return;

    // Extraer el dominio del email (ej. 'uab.cat' de 'alumno@uab.cat')
    const userDomain = email.split('@')[1].toLowerCase();

    // Consultar la lista de centros en la base de datos
    const centersSnap = await admin.database().ref('centers').once('value');
    const centers = centersSnap.val();

    let isValidDomain = false;

    // Iterar sobre los centros para comprobar si el dominio existe
    if (centers) {
        for (const key in centers) {
            // Se asume que tus objetos 'center' tienen un campo 'domain'
            if (centers[key].domain === userDomain) {
                isValidDomain = true;
                break;
            }
        }
    ...
    if (!isValidDomain) {
        // Si el dominio no pertenece a ningún centro registrado, ELIMINAR al usuario
        console.warn(`Usuario eliminado. Dominio no autorizado: ${userDomain}`);
        await admin.auth().deleteUser(user.uid);
    } else {
        // Crear el perfil del usuario en la base de datos de forma segura
